...
class WebPostoClient:
    ...
    def __init__(self, base_url: Optional[str] = None, api_key: Optional[str] = None):
        ...
        self.timeout = (
            180  # Aumentado para suportar requisições pesadas (ex: consultar_abastecimento)
        )
    ...
    def _make_request(
        ...
        try:
            params_log = {
                k: (v[:8] + "..." if k == "chave" and v else v) for k, v in params.items()
            }
            ...
            if response.status_code == 404:
                return {"success": False, "error": "Recurso não encontrado.", "status_code": 404}
